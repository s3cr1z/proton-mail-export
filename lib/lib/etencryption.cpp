...
void FileEncryptor::encryptFile(const std::filesystem::path& inputPath, 
    ...
    // Read entire file into memory (for simplicity - could be optimized for large files)
...
void BackupManifest::writeToFile(const std::filesystem::path& path) const {
    ...
    if (!file) {
        throw std::runtime_error("Failed to open manifest file for writing: " + path.string());
    }
...
BackupManifest BackupManifest::readFromFile(const std::filesystem::path& path) {
    ...
    if (!file) {
        throw std::runtime_error("Failed to open manifest file for reading: " + path.string());
    }
