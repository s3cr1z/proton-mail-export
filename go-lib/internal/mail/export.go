...
func NewExportTask(
        ...
        if exportConfig == nil {
                exportConfig = NewDefaultExportConfig()
        }
...
func (e *ExportTask) Run(ctx context.Context, reporter Reporter) error {
        ...
        buildStage := NewBuildStage(NumParallelBuilders, e.log, buildMemMB, e.session.GetPanicHandler(), e.session.GetReporter(), user.ID, e.exportConfig)
