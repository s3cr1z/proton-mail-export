...
var (
        ...
        flagNoTUI = &cli.BoolFlag{ //nolint:gochecknoglobals
                Name:    "no-tui",
                Usage:   "Disable TUI mode and use CLI interface",
                EnvVars: []string{"ET_NO_TUI"},
        }
        flagTUIMode = &cli.BoolFlag{ //nolint:gochecknoglobals
                Name:  "tui",
                Usage: "Force TUI mode (default when no args provided)",
        }
)
...
func Run() {
        ...
        app := &cli.App{
                ...
                Usage:  "Proton Mail Export Tool - Backup and restore your emails",
                Action: run,
                Flags: []cli.Flag{
                        flagUsername,
                        flagPassword,
...
func run(ctx *cli.Context) error {
        ...
        if ctx.Bool("tui") || (!ctx.Bool("no-tui") && shouldUseTUI(ctx)) {
                // Run TUI mode
                return runTUI(ctx)
        }

        // Run enhanced CLI mode
        return runEnhancedCLI(ctx)
}
